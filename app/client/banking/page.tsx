...
function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className={`p-6 ${product.stanbicPathway ? "bg-gradient-to-r from-[#0f4c8a] to-[#1a5fa8]" : "bg-gradient-to-r from-[#E9521C] to-[#d44a18]"} text-white`}>
          ...
          <div className="flex gap-3">
            <Button
              className={`flex-1 rounded-xl font-bold ${product.unlocked
                ? product.stanbicPathway ? "bg-[#0f4c8a] hover:bg-[#0a3a6e] text-white" : "bg-[#E9521C] hover:bg-[#d44a18] text-white"
    ...
    <div className="space-y-6 pb-10">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-semibold text-[#E9521C] uppercase tracking-widest">Banking Pathway</span>
        </div>
      ...
      <Card className="border border-gray-100 shadow-sm rounded-2xl">
        ...
        <CardContent>
          <div className="flex items-center justify-between relative">
            ...
            <div className="absolute left-0 top-5 h-0.5 bg-[#E9521C] z-0" style={{ width: "40%" }} />
            {pathway.map((s, i) => (
              <div key={s.step} className="flex flex-col items-center gap-2 z-10 flex-1">
                <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center ${
                  s.done ? "bg-[#E9521C] border-[#E9521C]" :
                  s.active ? "bg-white border-[#E9521C] shadow-md" :
                  "bg-white border-gray-200"
                }`}>
                  {s.done ? <CheckCircle className="w-5 h-5 text-white" /> :
                   s.active ? <div className="w-3 h-3 rounded-full bg-[#E9521C] animate-pulse" /> :
                   <Lock className="w-4 h-4 text-gray-300" />}
                </div>
                <p className={`text-[10px] font-semibold text-center leading-tight ${s.done ? "text-gray-600" : s.active ? "text-[#E9521C]" : "text-gray-300"}`}>
                  {s.label}
                </p>
              </div>
            ))}
              ...
              >
                <Card
                  className={`border rounded-2xl shadow-sm cursor-pointer transition-all hover:shadow-md ${
                    product.unlocked
                      ? "border-gray-100 hover:border-[#E9521C]/30"
