...
export default function ClientLoansPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-black flex items-center gap-2"><CreditCard className="h-6 w-6 text-[#E9521C]" />Loan Products</h1>
        <p className="text-gray-500 text-sm mt-1">Your credit score: <span className="font-semibold text-[#E9521C]">{creditScore}</span> · Medium eligibility</p>
      </div>
          ...
          <Card key={p.id} className="border-gray-200">
            <CardContent className="p-5">
              ...
              {applied === p.id ? (
                ...
                <Button onClick={() => handleApply(p.id)} className="w-full bg-[#E9521C] text-white hover:bg-black text-sm">Apply Now</Button>
              )}
            </CardContent>
          </Card>
        ))}
