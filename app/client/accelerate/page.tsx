  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-[#E9521C] uppercase tracking-widest">Accelerate Incubator</span>
          </div>
        ...
        <Badge className={`shrink-0 ${metCount === requirements.length ? "bg-green-100 text-green-700" : "bg-orange-100 text-[#E9521C]"} border-0 text-xs px-3 py-1.5`}>
      ...
      <div className="relative bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] rounded-3xl p-6 overflow-hidden shadow-xl">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#E9521C]/10 rounded-full blur-[80px]" />
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-[#E9521C]/5 rounded-full blur-[60px]" />
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <div>
            <p className="text-white/50 text-xs uppercase tracking-widest mb-2">Overall Readiness</p>
            <div className="w-full max-w-[200px]">
              <GaugeChart value={animated ? overallReadiness : 0} max={100} color="#E9521C" />
            </div>
            <div className="mt-1">
          ...
          <div className="space-y-3">
              ...
              <div key={r.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="h-1 bg-white/10 rounded-full mt-1 overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all duration-1000 ${r.met ? "bg-green-500" : "bg-[#E9521C]"}`}
            ...
            {overallReadiness === 100 && (
              <Button className="w-full mt-2 bg-[#E9521C] hover:bg-[#d44a18] text-white rounded-xl font-bold">
      ...
      <Card className="border border-gray-100 shadow-sm rounded-2xl overflow-hidden">
        ...
        <CardContent>
          <div className="flex items-center justify-between relative">
            ...
            <div
              className="absolute left-0 top-5 h-0.5 bg-[#E9521C] z-0 transition-all duration-1000"
              ...
              return (
                <div key={s.id} className="flex flex-col items-center gap-2 z-10 flex-1">
                  <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all ${
                    done ? "bg-[#E9521C] border-[#E9521C]" :
                    active ? "bg-white border-[#E9521C] shadow-lg shadow-[#E9521C]/20" :
                    "bg-white border-gray-200"
                  }`}>
                    {done ? (
                      <CheckCircle className="w-5 h-5 text-white" />
                    ) : active ? (
                      <div className="w-3 h-3 rounded-full bg-[#E9521C] animate-pulse" />
                    ) : (
                      <Lock className="w-4 h-4 text-gray-300" />
                    )}
                  </div>
                  <div className="text-center">
                    <p className={`text-xs font-bold leading-tight ${active ? "text-[#E9521C]" : done ? "text-gray-700" : "text-gray-300"}`}>
                      {s.label}
                    </p>
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${req.met ? "bg-green-100" : "bg-[#E9521C]/10"}`}>
                        <Icon className={`w-5 h-5 ${req.met ? "text-green-600" : "text-[#E9521C]"}`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {req.id !== "activity" && (
                          <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-1000 ${req.met ? "bg-green-500" : "bg-[#E9521C]"}`}
                        ...
                        >
                          <div className="mt-3 pt-3 border-t border-gray-100">
                            <div className="flex items-start gap-2">
                              <Zap className="w-3.5 h-3.5 text-[#E9521C] mt-0.5 shrink-0" />
      ...
      <Card className="border border-gray-100 shadow-sm rounded-2xl">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-bold text-gray-900 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-[#E9521C]" /> Upcoming Milestones
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-2">
          {upcomingMilestones.map((m, i) => (
            <div key={i} className={`flex items-center justify-between p-3 rounded-xl ${m.done ? "bg-green-50" : "bg-gray-50"}`}>
              ...
              <span className={`text-xs font-bold shrink-0 ml-2 ${m.done ? "text-green-600" : "text-[#E9521C]"}`}>{m.points}</span>
            </div>
          ))}
        </CardContent>
      </Card>
            ...
            >
              <Card className="border border-gray-100 shadow-sm rounded-2xl">
                <CardContent className="p-4 flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#E9521C]/10 flex items-center justify-center shrink-0 text-lg">
                    {b.icon}
                  </div>
                  <div>
