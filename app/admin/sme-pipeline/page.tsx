...
function BusinessProfileModal({ client, onClose }: { client: Client; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-[#E9521C] to-[#d44a18] p-6 text-white">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center justify-between p-4 bg-orange-50 rounded-xl border border-orange-100">
            <div>
              ...
              <p className="text-3xl font-black text-[#E9521C]">{client.score}<span className="text-base font-medium text-gray-400">/100</span></p>
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {client.products.map((p) => (
                <span key={p} className="px-3 py-1 bg-orange-50 text-[#E9521C] text-xs font-semibold rounded-full border border-orange-100">
                  {p}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center justify-between p-4 bg-navy-50 rounded-xl border border-gray-100">
            ...
            {client.status === "Pending Referral" && <Star className="w-6 h-6 text-[#E9521C]" />}
          </div>
...
export default function AdminSMEPipelinePage() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <Badge className="bg-orange-100 text-[#E9521C] border-0 text-xs">Pending Referral</Badge>;
  };
              ...
              <div className="flex items-center justify-between mt-2">
                <div className="flex gap-4">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#fde8e0] border border-[#E9521C]" />
                    <span className="text-xs text-gray-500">Referred</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#E9521C]" />
              ...
              {sectorData.map((s) => (
                <div key={s.sector}>
                  ...
                  <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-[#E9521C] transition-all" style={{ width: `${s.pct}%` }} />
                  </div>
                </div>
              ))}
            </CardContent>
            ...
            <CardContent className="space-y-2">
              {filteredClients.map((client) => (
                <div
                  ...
                  className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-[#E9521C]/30 hover:bg-orange-50/30 cursor-pointer transition-all group"
                >
                  <Avatar className="w-9 h-9 rounded-xl bg-[#E9521C]/10 shrink-0">
                    <AvatarFallback className="bg-[#E9521C]/10 text-[#E9521C] text-xs font-bold rounded-xl">
                      {client.initials}
                    </AvatarFallback>
                  </Avatar>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="text-right hidden sm:block">
                      <p className="text-xs font-black text-[#E9521C]">{client.score}</p>
                      <p className="text-xs text-gray-400">score</p>
                    </div>
                    {getStatusBadge(client.status)}
                    <ArrowRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-[#E9521C] transition-colors" />
                  </div>
                </div>
              ))}
